...
(function($) {
	...
	function initTextBox(target) {
		...
		$input.css({
			width: tw - $arrow.outerWidth(true) - parseFloat(p.left) - parseFloat(p.bottom),
		...
		if (state.panel.is(":hidden")) {
			...
			if (opts.dependenciesIds) {
				...
				for (; selector = opts.dependenciesIds[i]; i++) {
					...
					$.extend(true, opts.queryParams, vs);
				}
			}
	...
	function request(target, url, param, remainText) {
		...
		param = $.extend(true, {}, opts.queryParams, param);
	...
	function setTextBoxValue(state, vv, tt) {
		...
		$v.val(vv.join(opts.separator));
		$t.val(tt.join(opts.separator));
	}
	...
	function doEnter(target) {
		var $target = $(target),
			...
			opts = state.options,
			$panel = state.panel,
			$item = $panel.children().find("div.s-combobox-item-hover");
		if ($item.size()) {
	...
	function select(target, value) {
		...
		state.previous = $(target).combobox("getText");
